...
static const char *user_phone_arr[][2] = { { "user111", "phone111" },
                                           { "user222", "phone222" },
                                           { "user333", "phone333" },
                                           { NULL, NULL } };
...
int main()
{
    ...
    conn = PQconnectdb("user=redash password= host=127.0.0.1 dbname=vision");
    ...
    const char *query = "INSERT INTO phonebook (name, phone, last_changed) "
                        " VALUES ($1, $2, now());";
    const char *params[2];
    ...
    res = PQexec(conn, "SELECT id, unix, open, low FROM crypto");
    ...
    for (int i = 0; i < nrows; i++) {
        ...
        printf("Id: %s, UNIX: %s, OPEN: %s, LOW: %s\n", id, name, phone,
               last_changed);
    }
